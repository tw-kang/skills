...
- **Session cost — repeat runs in ONE session.** Each `ctp.sh … --interactive` invocation pays ~85s setup (JVM + DB create + server start/stop/delete); an extra `run` inside the same live session is only ~1.5s. So for a determinism/repeat check, send `printf 'run <case>\n'` N times before `quit` in a **single** invocation — **never** launch N separate `ctp.sh` processes (that multiplies the 85s). `.result` is overwritten each run.
